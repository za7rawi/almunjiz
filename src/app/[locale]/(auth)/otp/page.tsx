...
export default function OtpPage() {
  ...
  const otpType = typeof window !== 'undefined' ? sessionStorage.getItem('otp_type') || 'email' : 'email';
  const displayValue = typeof window !== 'undefined' ? sessionStorage.getItem('otp_phone_display') || '' : '';
  const devCode = typeof window !== 'undefined' ? sessionStorage.getItem('otp_dev_code') || '' : '';

  const maskedDisplay = displayValue
    ? otpType === 'email'
      ? displayValue.replace(/(.{3})(.*)(@.*)/, '$1***$3')
      : displayValue
    : otpType === 'email'
    ? '***@***.com'
    : '***';
  ...
  }, [identifier, otpType, router, login]);
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setErrorMsg('خدمة إرسال رمز التحقق غير مفعلة حالياً. يرجى المحاولة لاحقاً.');
      }
    } catch {
      setErrorMsg('خدمة إرسال رمز التحقق غير مفعلة حالياً. يرجى المحاولة لاحقاً.');
    } finally {
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible">
      <motion.div variants={itemVariants} className="text-center mb-8">
        ...
        <div className="flex items-center justify-center gap-2">
          {otpType === 'phone' ? (
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#14b8a6]"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" /></svg>
          ) : (
            <Mail size={16} className="text-[#14b8a6]" />
          )}
          <p className="text-white font-semibold text-base tracking-wider" dir="ltr">
            {maskedDisplay}
          </p>
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="flex justify-center gap-2.5 sm:gap-3 mb-8"
      >
        {otp.map((digit, i) => (
          <motion.div
            key={i}
            ...
            <input
              ...
              className={`w-11 h-14 sm:w-13 sm:h-16 text-center text-xl sm:text-2xl font-bold text-white bg-white/[0.05] border rounded-xl focus:outline-none transition-all duration-300 disabled:opacity-50 ${
                verified
                  ...
                  : digit
                  ? 'border-[#2580eb]/50 bg-[#2580eb]/5 shadow-lg shadow-[#2580eb]/10'
      ...
      <AnimatePresence>
        {error && errorMsg && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="flex items-center justify-center gap-2 mb-4 p-3 rounded-xl bg-red-500/10 border border-red-500/20"
          >
      ...
      <motion.div variants={itemVariants} className="mt-8 flex flex-col items-center gap-4">
        {!resendable ? (
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#2580eb]/10 border border-[#2580eb]/20 text-[#2580eb] text-sm font-semibold hover:bg-[#2580eb]/15 transition-all duration-200 disabled:opacity-50"
          >
