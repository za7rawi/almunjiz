...
import { ArrowRight, CheckCircle, FileText, MessageCircle, Package, Loader2, Clock } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
                  ...
                  return (
                    <motion.div key={step.status} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }} className="flex items-start gap-4">
                      <div className="flex flex-col items-center">
                        ...
                        {i < fullTimelineSteps.length - 1 && <div className={`w-0.5 h-8 transition-colors duration-300 ${isDone && i < currentStep ? 'bg-emerald-500' : 'bg-slate-200'}`} />}
                      </div>
                      <div className="pb-6">
                        <p className={`text-sm font-medium ${isDone ? 'text-slate-900' : isCurrent ? 'text-[#2580eb]' : 'text-slate-400'}`}>{step.label}{isCurrent && <Badge variant="primary" size="sm" className="ms-2">الحالية</Badge>}</p>
                      </div>
                    </motion.div>
                  )
                })}
          ...
          {timeline.length > 0 && (
            <Card>
              <CardHeader><h3 className="font-bold text-slate-900">سجل التحديثات</h3></CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {timeline.map((entry, i) => {
                    const cfg = statusConfig[entry.status] || { label: entry.status, variant: 'primary' as const }
                    return (
                      <motion.div key={entry.id || i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-400 mt-1">{new Date(entry.createdAt).toLocaleString('ar-SA', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                        </div>
                      </motion.div>
                    )
                  })}
          ...
          <Card>
            <CardContent>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-sm"><span className="text-slate-500">حالة الدفع</span><span className="font-medium text-slate-900">{(order.paymentStatus as string) || 'بانتظار الدفع'}</span></div>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader><h3 className="font-bold text-slate-900">تفاصيل الفاتورة</h3></CardHeader>
            <CardContent>
              <div className="space-y-3">
                <div className="flex justify-between text-sm"><span className="text-slate-500">المبلغ</span><span className="text-slate-700">{Number(order.amount as number || 0).toLocaleString()} ر.س</span></div>
                {Number(order.discount as number || 0) > 0 && <div className="flex justify-between text-sm"><span className="text-slate-500">الخصم</span><span className="text-emerald-600">-{Number(order.discount as number || 0).toLocaleString()} ر.س</span></div>}
                {Number(order.tax as number || 0) > 0 && <div className="flex justify-between text-sm"><span className="text-slate-500">الضريبة</span><span className="text-slate-700">{Number(order.tax as number || 0).toLocaleString()} ر.س</span></div>}
                <div className="h-px bg-slate-200 my-2" />
                <div className="flex justify-between text-sm"><span className="font-bold text-slate-900">الإجمالي</span><span className="font-bold text-slate-900">{Number(order.total as number || 0).toLocaleString()} ر.س</span></div>
              </div>
            </CardContent>
          </Card>
