...
export default function OrderDetailPage({ params }: OrderDetailProps) {
  ...
  const statusCfg = statusConfig[order.status] || { label: order.status, variant: 'primary' as const }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="pb-6">
                        ...
                        {timelineEntry?.date && (
                          <p className="text-xs text-slate-400 mt-0.5">{timelineEntry.date}</p>
                        )}
                      </div>
                    </motion.div>
                  )
          ...
          {order.description && (
            <Card>
              <CardHeader>
                <h3 className="font-bold text-slate-900">الوصف والملاحظات</h3>
              </CardHeader>
              <CardContent>
              ...
              <CardContent>
                <div className="space-y-3">
                  {order.attachments.map((file) => (
                    <div key={file} className="flex items-center justify-between p-3 rounded-xl bg-slate-50">
                      <div className="flex items-center gap-3">
                        <FileText size={18} className="text-slate-400" />
                        <span className="text-sm text-slate-700">{file}</span>
                      </div>
                      <button className="p-1.5 rounded-lg hover:bg-white text-[#2580eb] transition-colors">
                        <Download size={16} />
                      </button>
                    </div>
                  ))}
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-slate-700">{order.amount} ر.س</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500">الضريبة (15%)</span>
                  <span className="text-slate-700">{order.tax} ر.س</span>
                </div>
                <div className="h-px bg-slate-200 my-2" />
                <div className="flex justify-between text-sm">
                  <span className="font-bold text-slate-900">الإجمالي</span>
                  <span className="font-bold text-slate-900">{order.total} ر.س</span>
                </div>
              </div>
            </CardContent>
          </Card>
