  ...
  return (
    <div>
      <PageHeader
        ...
        breadcrumbs={[{ label: language === 'ar' ? 'لوحة التحكم' : 'Dashboard', href: '/dashboard' }, { label: language === 'ar' ? 'طلباتي' : 'My Orders' }]}
      />

      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="flex-1"><SearchInput placeholder={language === 'ar' ? 'بحث في الطلبات...' : 'Search orders...'} onSearch={setSearch} /></div>
      </div>

      <div className="flex gap-2 mb-6 overflow-x-auto pb-2 scrollbar-thin">
        {tabs.map((tab) => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${activeTab === tab.id ? 'bg-[#2580eb] text-white shadow-lg shadow-[#2580eb]/25' : 'bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:border-[#2580eb]/30'}`}>
            {language === 'ar' ? tab.labelAr : tab.labelEn}
            {tab.id !== 'all' && <span className="ms-1.5 text-xs opacity-70">({orders.filter(o => tab.id === 'all' || o.status === tab.id).length})</span>}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filteredOrders.map((order, i) => {
            ...
            return (
              <motion.div key={order.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} whileHover={{ y: -2 }} onClick={() => router.push(`/dashboard/orders/${order.id}`)} className="cursor-pointer">
                <Card padding="none" className="overflow-hidden hover:border-[#2580eb]/30 transition-all">
                  <div className="p-5">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#2580eb]/10 to-[#14b8a6]/10 flex items-center justify-center text-[#2580eb]"><Package size={22} /></div>
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="font-bold text-slate-900 dark:text-white">{order.orderNumber}</h4>
                            <Badge variant={st.variant} size="sm" dot>{language === 'ar' ? sl.ar : sl.en}</Badge>
                          </div>
                          <p className="text-sm text-slate-500 mt-0.5">{order.service?.name || '-'}</p>
                        </div>
                      </div>
                    ...
                    <div className="mt-3 sm:hidden"><Progress value={st.progress} height="sm" /></div>
                  </div>
                </Card>
              </motion.div>
            )
