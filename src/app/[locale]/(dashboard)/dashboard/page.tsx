...
export default function DashboardPage() {
  ...
  return (
    <div>
      <PageHeader title={`${language === 'ar' ? 'مرحباً' : 'Welcome'}, ${user?.name?.split(' ')[0] || (language === 'ar' ? 'مستخدم' : 'User')} 👋`} subtitle={language === 'ar' ? 'إليك نظرة عامة على حسابك' : "Here's an overview of your account"} gradient />
                    ...
                    return (
                      <motion.div key={order.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} onClick={() => router.push(`/dashboard/orders/${order.id}`)} className="flex items-center justify-between px-5 py-3.5 hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors cursor-pointer">
                        <div className="flex items-center gap-3">
                          ...
                          <div><p className="text-sm font-medium text-slate-900 dark:text-white">{order.orderNumber}</p><p className="text-xs text-slate-500">{order.service?.name || '-'}</p></div>
                        </div>
