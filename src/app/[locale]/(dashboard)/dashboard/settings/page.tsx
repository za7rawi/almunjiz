...
function ToggleSwitch({
  ...
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[#2580eb]/30 ${
        checked ? 'bg-[#2580eb]' : 'bg-slate-200'
      }`}
    >
      <motion.span
        layout
...
export default function SettingsPage() {
  ...
  const handleDeleteAccount = () => {
    ...
    localStorage.removeItem('almunjiz-auth')
    router.replace('/')
  }
          ...
          <AnimatePresence mode="wait">
                      ...
                      <div className="flex items-center gap-4 mb-6">
                        <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#2580eb] to-[#14b8a6] flex items-center justify-center text-white text-2xl font-bold overflow-hidden">
                          {avatar ? (
                            ...
                            getInitials(name || 'مستخدم')
                          )}
                        </div>
                        <div>
                          <button
                            onClick={() => fileInputRef.current?.click()}
                            className="text-sm text-[#2580eb] hover:underline flex items-center gap-1.5"
                          >
                            <Upload size={14} />
                            تغيير الصورة
                          </button>
            ...
            {activeTab === 'notifications' && (
              ...
              >
                <Card>
                  <CardHeader>
                    <h3 className="font-bold text-slate-900">تفضيلات الإشعارات</h3>
                  </CardHeader>
                  <CardContent>
                      ...
                      {[
                        {
                          ...
                          desc: 'تلقى إشعارات عبر البريد الإلكتروني',
                        },
                        {
                          key: 'mobile' as const,
                          label: 'إشعارات الجوال',
                          desc: 'تلقى إشعارات فورية على جهازك',
                        },
                        {
                          key: 'orders' as const,
                          label: 'إشعارات الطلبات',
                          desc: 'تلقى إشعارات عند تحديث حالة طلبك',
                        },
                        {
                          key: 'offers' as const,
                          label: 'إشعارات العروض',
                          desc: 'تلقى إشعارات العروض الخاصة والخصومات',
                        },
                      ].map((item) => (
                        <div
                          key={item.key}
                          className="flex items-center justify-between p-4 rounded-xl bg-slate-50 hover:bg-slate-100/80 transition-colors"
                        >
                          <div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-red-600 mt-1">
                            حذف حسابك سيمسح جميع بياناتك نهائياً بما في ذلك {orders.length} طلب.
                            لا يمكن التراجع عن هذا الإجراء.
                          </p>
                        </div>
                      </div>
                    </div>
                    <Button variant="danger" size="md" onClick={handleDeleteAccount} iconLeft={<Trash2 size={16} />}>
                      حذف الحساب نهائياً
                    </Button>
                  </CardContent>
                </Card>
