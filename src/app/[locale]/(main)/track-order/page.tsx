...
const orderStatusConfig: Record<string, { label: string; color: string }> = {
  ...
  WAITING_CLIENT: { label: 'بانتظار العميل', color: 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300' },
...
export default function TrackOrderPage() {
  ...
  const sc = foundOrder ? (orderStatusConfig[foundOrder.status] || { label: foundOrder.status, color: 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300' }) : null;
  const pc = foundOrder?.paymentStatus ? (paymentStatusConfig[foundOrder.paymentStatus] || { label: foundOrder.paymentStatus, color: 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300' }) : null;
        ...
        {searching && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <Card glass className="p-12 text-center">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">جاري البحث عن الطلب...</h3>
              <p className="text-slate-500 dark:text-slate-400 text-sm">يرجى الانتظار قليلاً</p>
            </Card>
          </motion.div>
        )}
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <Card glass className="p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white">الطلب #{foundOrder.orderNumber}</h3>
                  {foundOrder.invoice?.invoiceNumber && (
                    <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 font-mono" dir="ltr">الفاتورة: {foundOrder.invoice.invoiceNumber}</p>
                  )}
                </div>
                ...
                {foundOrder.customerName && (
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white">{foundOrder.customerName}</p>
                    </div>
                  </div>
                )}
                {foundOrder.customerEmail && (
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white" dir="ltr">{foundOrder.customerEmail}</p>
                    </div>
                  </div>
                )}
                {foundOrder.customerPhone && (
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white" dir="ltr">{foundOrder.customerPhone}</p>
                    </div>
                  </div>
                )}
                {foundOrder.service?.name && (
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white">{foundOrder.service.name}</p>
                    </div>
                  </div>
                )}
                {foundOrder.baseAmount != null && foundOrder.baseAmount > 0 && (
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white">{formatPrice(foundOrder.baseAmount, currency)}</p>
                    </div>
                  </div>
                )}
                {foundOrder.discount != null && foundOrder.discount > 0 && (
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white">{formatPrice(foundOrder.discount, currency)}</p>
                    </div>
                  </div>
                )}
                {foundOrder.tax != null && foundOrder.tax > 0 && (
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white">{formatPrice(foundOrder.tax, currency)}</p>
                    </div>
                  </div>
                )}
                {foundOrder.total != null && foundOrder.total > 0 && (
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-900 dark:text-white">{formatPrice(foundOrder.total, currency)}</p>
                    </div>
                  </div>
                )}
                {foundOrder.paymentMethod && (
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white">{foundOrder.paymentMethod}</p>
                    </div>
                  </div>
                )}
                {foundOrder.transactionId && (
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white font-mono" dir="ltr">{foundOrder.transactionId}</p>
                    </div>
                  </div>
                )}
                <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-900 dark:text-white">{new Date(foundOrder.createdAt).toLocaleDateString('ar-SA')}</p>
                  </div>
                </div>
              </div>
            </Card>
                    ...
                    <div key={entry.id || i} className="flex items-start gap-4">
                      <div className="flex flex-col items-center">
                        ...
                        {i < (foundOrder.timeline?.length || 0) - 1 && (
                          <div className="w-0.5 h-8 bg-slate-200 dark:bg-slate-600" />
                        )}
                      </div>
                      <div className="pb-6 flex-1">
                        <p className="text-sm font-semibold text-slate-900 dark:text-white">{entry.status}</p>
                        {entry.description && (
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{entry.description}</p>
                        )}
                  ...
                  {foundOrder.fileAttachments.map((file) => (
                    <div key={file.id} className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden hover:shadow-md transition-shadow">
                      {isImageFile(file.mimeType) ? (
                        ...
                        <div className="h-32 bg-red-50 dark:bg-red-900/20 flex items-center justify-center">
        ...
        {error && !searching && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <Card glass className="p-12 text-center">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">تأكد من رقم الطلب وحاول مرة أخرى</p>
              <button
        ...
        {!foundOrder && !searching && !error && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <Card glass className="p-12 text-center">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">ابحث عن طلبك</h3>
              <p className="text-slate-500 dark:text-slate-400 text-sm">أدخل رقم الطلب في مربع البحث أعلاه لمتابعة حالته</p>
            </Card>
          </motion.div>
        )}
      </div>
